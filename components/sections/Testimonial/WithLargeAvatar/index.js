...
const BackgroundContainer = tw.section`bg-white overflow-hidden`;
const Container = styled.div`${tw`relative max-w-7xl mx-auto pt-20 pb-12 px-4 sm:px-6 lg:px-8 lg:py-20`}
  .pattern1 {
...
`;
const Testimonial = styled.div`${tw`relative lg:flex lg:items-center`}
  .imageWrap {
    ${tw`hidden lg:block lg:flex-shrink-0`}
    img {${tw`h-64 w-64 rounded-full xl:h-80 xl:w-80`}}
  }
  .blockquoteSvg {
    ${tw`absolute top-0 left-0 transform -translate-x-8 -translate-y-24 h-36 w-36 text-indigo-200 opacity-50`}
  }
  .text {
    ${tw`text-2xl leading-9 font-medium text-gray-900`}
  }
  .name {
    ${tw`text-base font-medium text-gray-900`}
  }
  .company {
    ${tw`text-base font-medium text-brand`}
  }
`;
